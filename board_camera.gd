...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventPanGesture:		
		set_zoom_level(zoom_level + sign(event.delta.y)*GameSettings.zoom_factor)
	if event.is_action("ZoomIn"):
		set_zoom_level(zoom_level + GameSettings.zoom_factor)
...
func set_zoom_level(level: float, mouse_world_position = self.get_global_mouse_position()):
	...
	zoom_level = clampf(level, GameSettings.min_zoom, GameSettings.max_zoom)
